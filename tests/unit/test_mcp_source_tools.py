"""Unit tests for the metatron_source_* MCP tools.
...
from metatron.mcp.tools import (
    _source_deps,
    source_create,
    source_delete,
    source_list,
...
async def test_schemas_returns_connectors_only():
    out = await source_schemas.metatron_source_schemas()
...
async def test_list_returns_workspace_sources_masked(monkeypatch):
    ...
    out = await source_list.metatron_source_list()
    assert "error" not in out
    assert out["count"] == 1
...
async def test_list_scopes_by_workspace(monkeypatch):
    ...
    out = await source_list.metatron_source_list("default")
...
async def test_create_valid_connector(monkeypatch):
    store = FakeConnStore()
    _patch_resolve(monkeypatch, store)
    monkeypatch.setattr("metatron.connectors.connection_sync.ensure_workspace_exists", _async_noop)

    out = await source_create.metatron_source_create(
        "confluence",
        "Docs",
...
async def test_create_rejects_channel_type(monkeypatch):
    ...
    out = await source_create.metatron_source_create("telegram", "Bot", {"bot_token": "t"})
    assert "error" in out
...
async def test_create_rejects_unknown_type(monkeypatch):
    ...
    out = await source_create.metatron_source_create("sap", "X", {})
    assert "error" in out
    assert store.created == []
...
async def test_create_missing_required_field(monkeypatch):
    ...
    out = await source_create.metatron_source_create("confluence", "Docs", {"url": "https://x"})
    assert "error" in out
    assert store.created == []
...
async def test_update_name_and_enabled(monkeypatch):
    ...
    out = await source_update.metatron_source_update("c1", name="Renamed", enabled=False)
    assert "error" not in out
...
async def test_update_config_passes_through_without_premerge(monkeypatch):
    ...
    out = await source_update.metatron_source_update(
        "c1",
...
async def test_update_cross_workspace_not_found(monkeypatch):
    ...
    out = await source_update.metatron_source_update("c1", name="X")
    assert "error" in out
...
async def test_update_no_fields(monkeypatch):
    ...
    out = await source_update.metatron_source_update("c1")
    assert "error" in out
...
async def test_delete_removes_connection(monkeypatch):
    ...
    out = await source_delete.metatron_source_delete("c1")
    assert "error" not in out
...
async def test_sync_starts_background_task(monkeypatch):
    ...
    monkeypatch.setattr("metatron.connectors.connection_sync.run_connection_sync", _fake_run)

    sentinel_bus = object()
    monkeypatch.setattr("metatron.mcp.server.get_activity_bus", lambda: sentinel_bus)

    out = await source_sync.metatron_source_sync("c1")
    assert "error" not in out
