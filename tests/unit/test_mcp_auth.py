"""Tests for metatron.mcp.auth — API key validation."""
...
from metatron.mcp.auth import get_api_key, require_api_key, validate_api_key


class TestGetApiKey:
    def test_returns_env_value(self) -> None:
        with patch.dict(os.environ, {"METATRON_MCP_API_KEY": "secret"}):
            assert get_api_key() == "secret"

    def test_returns_none_when_unset(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            # Remove the key if present
            os.environ.pop("METATRON_MCP_API_KEY", None)
            assert get_api_key() is None


class TestValidateApiKey:
    def test_no_key_configured_allows_all(self) -> None:
        with patch("metatron.mcp.auth.get_api_key", return_value=None):
    ...
    def test_missing_header_rejects(self) -> None:
        with patch("metatron.mcp.auth.get_api_key", return_value="secret"):
