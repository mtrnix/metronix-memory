...
* ``METRONIX_FRESHNESS_HEARTBEAT_TTL_SECONDS``
* ``METRONIX_FRESHNESS_RECLAIM_INTERVAL_ITERATIONS``
* ``METRONIX_FRESHNESS_SCHEDULED_SCAN_ENABLED``
* ``METRONIX_FRESHNESS_SCHEDULED_SCAN_INTERVAL_SECONDS``
* ``METRONIX_FRESHNESS_SCAN_BATCH_LIMIT``
* ``METRONIX_FRESHNESS_DRAIN_LEGACY_AT_STARTUP``
...
class TestEnvOverrides:
    def test_heartbeat_ttl_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("METRONIX_FRESHNESS_HEARTBEAT_TTL_SECONDS", "45")
        s = Settings()
        assert s.freshness_heartbeat_ttl_seconds == 45

    def test_reclaim_interval_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("METRONIX_FRESHNESS_RECLAIM_INTERVAL_ITERATIONS", "5")
        s = Settings()
    ...
    def test_scheduled_scan_enabled_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("METRONIX_FRESHNESS_SCHEDULED_SCAN_ENABLED", "false")
        s = Settings()
    ...
    def test_scheduled_scan_interval_seconds_from_env(
        ...
        monkeypatch.setenv("METRONIX_FRESHNESS_SCHEDULED_SCAN_INTERVAL_SECONDS", "60")
        s = Settings()
    ...
    def test_scan_batch_limit_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("METRONIX_FRESHNESS_SCAN_BATCH_LIMIT", "100")
        s = Settings()
        assert s.freshness_scan_batch_limit == 100

    def test_drain_legacy_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("METRONIX_FRESHNESS_DRAIN_LEGACY_AT_STARTUP", "true")
        s = Settings()
...
class TestBoolParsing:
    ...
    def test_scheduled_scan_enabled_false_variants(
        ...
        monkeypatch.setenv("METRONIX_FRESHNESS_SCHEDULED_SCAN_ENABLED", value)
        s = Settings()
        assert s.freshness_scheduled_scan_enabled is False

    @pytest.mark.parametrize("value", ["true", "TRUE", "1", "yes", "on"])
    def test_drain_legacy_true_variants(self, monkeypatch: pytest.MonkeyPatch, value: str) -> None:
        monkeypatch.setenv("METRONIX_FRESHNESS_DRAIN_LEGACY_AT_STARTUP", value)
        s = Settings()
        assert s.freshness_drain_legacy_at_startup is True
