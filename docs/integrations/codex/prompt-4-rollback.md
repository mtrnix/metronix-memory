...
## 1. Downgrade or remove the routing rule (AGENTS.md)
Open the `AGENTS.md` that prompt 2 edited (`~/.codex/AGENTS.md` for a
user-scope install, or `<project>/AGENTS.md` in the project root for project
scope).
...
- confirm the `metronix-config` block now has the OPTIONAL wording and that all
  pre-existing content in AGENTS.md is still present and unchanged
