...
Test-only hook: ``METRONIX_FRESHNESS_TEST_WORKER_ID`` pins the id so
...
def build_worker_id() -> str:
    ...
    Respects the ``METRONIX_FRESHNESS_TEST_WORKER_ID`` env override when
    set to a non-empty value; otherwise composes
    ``{hostname}:{pid}:{uuid4_prefix}``.
    """
    override = os.environ.get("METRONIX_FRESHNESS_TEST_WORKER_ID")
    if override:
        return override
