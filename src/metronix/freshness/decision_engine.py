"""DecisionEngine — classifies a record for the freshness pipeline.

Moved from ``metronix.memory.freshness.decision_engine`` in Phase B
(MTRNIX-313). ``apply_decision`` was extracted to
:mod:`metronix.freshness.apply_decision` so it can take a
:class:`FreshnessTarget` instead of a concrete memory PG store, and so
both memory and KB pipelines share one implementation.

Two engine implementations:

* :class:`RuleBasedDecisionEngine` — zero-LLM heuristic. Returns confidence
  around 0.55 based on simple keyword extraction. Used when no
  ``METRONIX_FRESHNESS_LLM_API_BASE_URL`` is configured.
* :class:`LLMBackedDecisionEngine` — calls an SLM (e.g. qwen2.5-4b) through
  the existing sync ``LLMProvider`` via ``asyncio.to_thread``. On malformed
  output or provider failure it falls back to ``RuleBasedDecisionEngine``.

The protocol lives in this module (not in :mod:`metronix.core.interfaces`) —
promote later when the shape has been stable across a release cycle.
"""
...
from metronix.core.models import FreshnessDecision
...
class DecisionEngine(Protocol):
    """Async protocol shared by memory and KB pipelines."""
