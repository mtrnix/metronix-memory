...
``metronix.memory.freshness.linker`` in Phase A.
...
from metronix.core.models import MachineEvent
...
def link_memory_items_batch(
    ...
    Phase A shim: forwards to :mod:`metronix.storage.memory_graph`. Kept here
    so existing imports (``from metronix.memory.freshness.linker import
    link_memory_items_batch``) still resolve via the re-export shim.
    """
    from metronix.storage.memory_graph import link_memory_items_batch as _batch
