"""Sync Metronix users to Open WebUI (bundled scenario).

Uses admin@metronix.local credentials (same as core's seed admin)
...
class OpenWebUISync:
    """Handles user sync from Metronix -> Open WebUI."""

    def __init__(
        self,
        owui_url: str,
        metronix_url: str,
        admin_email: str = "admin@metronix.local",
        admin_password: str = "metronix",
    ) -> None:
        self._owui_url = owui_url
        self._metronix_url = metronix_url
    ...
    def _map_role_to_owui(self, metronix_role: str) -> str:
        return "admin" if metronix_role == "admin" else "user"

    async def ensure_admin(self) -> None:
        """Ensure admin@metronix.local exists in Open WebUI.
    ...
    async def sync_user_created(
        ...
        try:
            ...
            if owui_token and self._metronix_url:
                await self._client.set_direct_connection(
                    user_token=owui_token,
                    metronix_url=self._metronix_url,
                    api_key=api_key,
                )
