...
class _SafeOpenAIChat(OpenAIChat):
    ...
    ``autoe`` CLIs do, which Metronix does not use). It surfaces afterwards, when
    Metronix reads the accumulated usage via :meth:`count_tokens_used`:
...
class BenchmarkGenerator:
    ...
    def from_settings(cls, settings: Settings) -> BenchmarkGenerator:
        """Create a generator from Metronix Core settings.
