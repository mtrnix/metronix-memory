"""MCP Server instance for Metronix.

This module provides the FastMCP server that exposes Metronix's knowledge
...
from metronix.core.events import ERROR_OCCURRED, TOOL_CALLED
...
# tool is wrapped. Tools are registered later via `import metronix.mcp.tools`
...
def get_transport() -> str:
    ...
    """
    return os.environ.get("METRONIX_MCP_TRANSPORT", TRANSPORT_STDIO)


def get_http_host() -> str:
    """Get the HTTP host from environment or arguments."""
    return os.environ.get("METRONIX_MCP_HOST", DEFAULT_HOST)


def get_http_port() -> int:
    """Get the HTTP port from environment or arguments."""
    return int(os.environ.get("METRONIX_MCP_PORT", str(DEFAULT_PORT)))
...
async def run_stdio() -> None:
    ...
    # Load workspace config from ~/.metronix/config.json
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Metronix MCP Server")
    parser.add_argument(
        "--transport",
        choices=[TRANSPORT_STDIO, TRANSPORT_HTTP],
        default=None,
        help="Transport type (default: from METRONIX_MCP_TRANSPORT env var)",
    )
    parser.add_argument(
        "--host",
        default=None,
...
if __name__ == "__main__":
    ...
    if args.transport:
        os.environ["METRONIX_MCP_TRANSPORT"] = args.transport
    if args.host:
        os.environ["METRONIX_MCP_HOST"] = args.host
    if args.port:
        os.environ["METRONIX_MCP_PORT"] = str(args.port)
