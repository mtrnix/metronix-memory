"""MCP tool: metronix_memory_update — update an existing memory record in place."""
...
async def metronix_memory_update(
    ...
    try:
        ...
        if content is None and tags is None and importance_score is None and kind is None:
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message=(
                        "metronix_memory_update: at least one of "
        ...
        try:
            ...
            logger.warning(
                "metronix_memory_update.qdrant_sync_failed",
                record_id=record_id,
                exc_info=True,
            )
        ...
        try:
            ...
            logger.warning(
                "metronix_memory_update.neo4j_sync_failed",
                record_id=record_id,
                exc_info=True,
            )
        ...
        error = handle_tool_error("metronix_memory_update", exc)
