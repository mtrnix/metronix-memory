"""MCP tool: metronix_memory_list — list agent memory records with pagination."""
...
from metronix.mcp.tools.models import MemoryListResponse, MemoryRecordDTO
...
async def metronix_memory_list(
    agent_id: str,
    ...
    try:
        if not agent_id:
            return {
                "error": MCPError(
                    ...
                    message="metronix_memory_list: agent_id is required",
        ...
        if not is_valid_agent_id(agent_id):
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message="metronix_memory_list: agent_id must be 1-64 chars of A-Za-z0-9._-",
                    hint="Use the same id you registered / send as the X-Agent-Id header",
                ).to_dict(),
            }
        ...
        try:
            ...
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message=f"metronix_memory_list: {exc}",
                ).to_dict(),
            }
        ...
        error = handle_tool_error("metronix_memory_list", exc)
