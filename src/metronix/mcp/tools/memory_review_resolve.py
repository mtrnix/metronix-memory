"""MCP tool: metronix_memory_review_resolve — apply resolution to a review entry.
...
async def metronix_memory_review_resolve(
    review_id: str,
    action: str,
    ...
    try:
        if not review_id:
            return {
                "error": MCPError(
                    ...
                    message="metronix_memory_review_resolve: review_id is required",
                ).to_dict(),
            }
        if not action:
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message="metronix_memory_review_resolve: action is required",
        ...
        try:
            ...
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message=f"metronix_memory_review_resolve: {exc}",
                ).to_dict(),
            }

        logger.info(
            "metronix_memory_review_resolve.done",
            workspace_id=ws_id,
        ...
        error = handle_tool_error("metronix_memory_review_resolve", exc)
