"""MCP tool: metronix_memory_get_context — assemble agent memory context for LLM injection."""
...
from metronix.mcp.tools.models import MemoryContextResponse
...
async def metronix_memory_get_context(
    agent_id: str,
    workspace_id: str,
    query: str,
    memory_top_k: int = 10,
    ...
    try:
        ...
        if not workspace_id:
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message="metronix_memory_get_context: workspace_id is required",
                ).to_dict(),
            }
        if not query or not query.strip():
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message="metronix_memory_get_context: query is required",
                ).to_dict(),
            }
        ...
        error = handle_tool_error("metronix_memory_get_context", exc)
