"""MCP tool: metronix_memory_search — hybrid agent memory search."""
...
async def metronix_memory_search(
    query: str,
    agent_id: str,
    ...
    try:
        if not query or not query.strip():
            return {
                "error": MCPError(
                    ...
                    message="metronix_memory_search: query is required",
        ...
        if not agent_id:
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message="metronix_memory_search: agent_id is required",
        ...
        if not is_valid_agent_id(agent_id):
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message="metronix_memory_search: agent_id must be 1-64 chars of A-Za-z0-9._-",
        ...
        try:
            ...
            return {
                "error": MCPError(
                    code=ErrorCode.INVALID_PARAMS,
                    message=f"metronix_memory_search: {exc}",
                ).to_dict(),
            }
        ...
        error = handle_tool_error("metronix_memory_search", exc)
